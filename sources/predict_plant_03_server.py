...
def output_prediction(image_path, save_dir = None, true_class = None, show_img = False):
    ...
    if show_img:
        title = 'Predicted class: {predicted_class}, prob = {class_prob} \n True Class: {true_class}'.format(predicted_class=predicted_class,class_prob=class_prob,true_class=true_class)
...
#
# This makes the classifier a continuously running server
#
while True:          #Loop forever (or until killed...)
  image = input()    #This takes a line of input, which should be the filename of the candidate image
  predicted_class, class_prob, true_class = output_prediction(image_path = image)  #Run the prediction
  #Now print the output.   In actual use, we'll connect to this via popen, rather than command line
  #But command line works just fine for debugging.
